...
layui.define(["jquery"], function (exports) {
  ...
  Node.prototype = {
    ...
    init: function () {
      ...
      this.$li = $('<li role="menuitem" id="cascader-menu-'+this.value+'" tabindex="-1" class="el-cascader-node" aria-haspopup="true" aria-owns="cascader-menu"><span class="el-cascader-node__label" title="'+label+'">' + label + '</span><i class="' + fromIcon + ' ' + icon + '"></i></li>');
    ...
    _liLoad: function (event, callback) {
      ...
      var self = this;
      var level = this.level;
      var offset = $('.el-cascader').offset();
      if(event.type=="mouseenter"){
          var remark = this.remark;
          if(!remark){
              remark= '暂时描述';
          }
          layer.tips(remark, '#cascader-menu-'+this.value,{
              tips:3,
              time:1500,
              success:function(layero,index){
                  if(level==1){
                      layero.css('left',offset.left+197)
                  }else{
                      layero.css('left',offset.left)
                  }

                  layero.css('top',offset.top)
                  layero.css('z-index',109891025)
                  layero.find('div').find('i').removeClass('layui-layer-TipsB').addClass('layui-layer-TipsT');
              }
          });
      }
      var cascader = this.cascader;
