...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements MenuService {
    ...
    @Override
    public Map<String, Object> findMenus(int roleId) {
        Map<String, Object> result = new HashMap<>();
        try {
            LambdaQueryWrapper<Menu> queryWrapper = new LambdaQueryWrapper<>();
            List<Menu> menus = baseMapper.selectList(queryWrapper);

            List<RoleMenu> roleMenusByRoleId = roleMenuServie.getHasPathRoleMenusByRoleId(roleId);
            List<Integer> menuIdList = roleMenusByRoleId.stream().map(RoleMenu::getMenuId).collect(Collectors.toList());
            List<Tree<Menu>> trees = new ArrayList<>();
            buildTrees(trees, menus);
            result.put("ids", menuIdList);
